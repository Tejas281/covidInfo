...
const Juice = () => {
  ...
  useEffect(() => {
    const fetchProduct = async () => {
      const { data } = await axios.get('https://covidjuicebhavnagar.herokuapp.com/covidjuice')
      setProducts(data)
    }
    fetchProduct()
  }, [])
        ...
        <table class="min-w-full divide-y divide-gray-200">
          ...
          {products.map((product) => (
            ...
            {product.fruitname}
              ...
              <td class="px-2 py-4 ">
                <span class="px-1 inline-flex text-xs leading-5 font-semibold rounded-full  text-green-800">
                  {product.description}
                </span>
              </td>
